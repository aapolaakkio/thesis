...
We solve Problem~\ref{prob:matching} by utilizing maximum flow computation with the Ford–Fulkerson algorithm~\citep{ford1957simple}. Ford–Fulkerson algorithm is used for computing the maximum flow through a directed graph. Ford–Fulkerson constructs a copy of the original graph, a so called residual graph, which it uses to calculate the maximum flow by repeatedly looking for augmenting paths through the graph. An augmenting path is a path through the graph that has capacity left. We use the a variant of the Ford–Fulkerson algorithm called Edmonds–Karp algorithm~\citep{edmonds1972theoretical}, which computes the augmenting paths using breadth-first search. Algorithm~\ref{alg:ff} gives the pseudo-code of the algorithm. We record the wall-clock time of the matching process so that the Ford–Fulkerson runtime can be reported alongside the solution.
...
After the matching step, we solve the schedules of each driver separately using integer linear programming. The matching subproblem provides, for each driver $k$, the set $A_k$ of customers that $k$ is allowed to serve.
...
It is possible that after removing the contested customers from the assignable sets of the drivers, previously unserved customers could now be assigned. Due to this, we run an iterated resolve of the per-driver scheduling problems. In each round, every driver's ILP is rerun on the union of its current assignments and the customers that are still unserved but lie in the driver's assignable set. Any customer a resolved ILP newly assigns is removed from the unserved pool immediately. We stop when a round changes no driver's assignments, when the unserved customer pool fails to shrink between rounds or after a configurable round limit, whichever comes first.
  ...
  \For{$\mathrm{round} = 1, \dots, \mathrm{round\ limit}$} \Comment{iterated re-solve}
    \ForAll{drivers $k$ in the order $\sigma$}
      ...
      \If{$C_k$ differs from the input of $k$'s previous solve}
        \State re-solve driver $k$'s ILP over $C_k$
